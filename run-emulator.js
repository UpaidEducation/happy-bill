...
const ANDROID_EMULATOR_PATH = `${process.env.ANDROID_HOME}/emulator/emulator`;
...
const emulators = execFileSync(ANDROID_EMULATOR_PATH, ['-list-avds'], { encoding: 'utf8' })
  .replace(/\n$/, '')
  .split('\n');

if (emulators.length > 0) {
  const emus = emulators.map(emu => emu.endsWith('\r') ? emu.slice(0, emu.length - 1) : emu)
  prompt([
    {
      type: 'list',
      name: 'emulator',
...
`)
    ...
    try {
      const runOnEmulator = spawnSync(ANDROID_EMULATOR_PATH, [
        '-avd',
        answer.emulator,
        answer.wipeData === 'Yes' ? '-wipe-data' : '-no-snapshot'
      ]);
